...
for %%i in (oaBrowser22.bat) do set rlocation=%%~$PATH:i
if not "%rlocation%" == "" goto foundme
for %%i in ("%0") do set rlocation=%%~dpioaBrowser22.bat
if not exist "%rlocation%" (
  echo ** Can't find: oaBrowser22.bat
  set rlocation=
  exit /B 1
)
...
:ready
for %%i in ("%rlocation%") do set slocation=%%~dpioaBrowser22.lpy
if not exist "%slocation%" (
  echo %slocation%
  echo ** Can't find: oaBrowser22.lpy
  set rlocation=
  set plocation=
  set slocation=
  exit /B 1
